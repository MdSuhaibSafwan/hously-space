...
class NonlineUrlsPipeline:
    ...
    async def process_item(self, item, spider):
        try:
            ...
            ad_exists = await sync_to_async(NonlineListings.objects.filter(url=item['url']).exists)()
            if not ad_exists:
                ad = NonlineListings(url=item['url'])  # Tylko URL
...
class NonlineDetailsPipeline:
    async def process_item(self, item, spider):
        if 'id' in item:
            try:
                ad = await sync_to_async(NonlineListings.objects.get)(id=item['id'])
            ...
            except NonlineListings.DoesNotExist:
...
class NonlineDownloadImagesPipeline:
    async def process_item(self, item, spider):
