...
IMAGES_DIR = os.path.join(settings.MEDIA_ROOT, 'images/nonline_images')

# Upewnij się, że folder istnieje
os.makedirs(IMAGES_DIR, exist_ok=True)

class NonlineDownloadImagesSpider(scrapy.Spider):
    name = 'nonline_images'
    allowed_domains = ['*']
    custom_settings = {
        ...
        'CONCURRENT_REQUESTS': 48,  # Zwiększenie liczby równoległych żądań
        'DOWNLOAD_DELAY': 0.15,  # Ustawienie opóźnienia między żądaniami
    }

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Pobierz ogłoszenia, które mają niepuste pole original_image_urls i puste pole images
        self.ads = list(NonlineListings.objects.exclude(original_image_urls__isnull=True).exclude(original_image_urls__exact='').exclude(isActive=False).filter(
            Q(images__isnull=True) | Q(images=[])).order_by('id'))
    ...
    async def download_images(self, response):
        ...
        for index, url in enumerate(image_urls):
            ...
            image_filename = f"nonline_{ad_id}_{index}.webp"
            image_save_path = os.path.join(IMAGES_DIR, image_filename)

            if self.download_image(url.split(';')[0], image_save_path):
                local_image_paths.append(os.path.join('images/nonline_images', image_filename))
            else:
    ...
    def update_listing(self, ad_id, local_image_paths, inactive):
        with transaction.atomic():
            listing = NonlineListings.objects.get(id=ad_id)
